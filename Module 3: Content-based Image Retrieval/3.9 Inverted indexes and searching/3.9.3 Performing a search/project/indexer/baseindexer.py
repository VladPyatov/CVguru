...
class BaseIndexer(object):
    def __init__(self, dbPath, estNumImages=500, maxBufferSize=50000, dbResizeFactor=2, verbose=True):
        # store the database path, estimated number of images in the dataset, max buffer size
        # the resize factor of the database and the verbosity setting
        self.dbPath = dbPath
    ...
    def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse=False):
        # if the buffer is a list, then compute the ending index based on the list length
        if type(buf) is list:
        ...
        if end > dataset.shape[0]:
            self._debug(f"triggering '{datasetName}' db resize")
            self._resizeDataset(dataset, datasetName, baseSize=end)

        # if this is the sparse matrix, then convert the sparse matrix to
        # a dense one so it can be written to file
        if sparse:
            buf = buf.toarray()

        # dump the buffer to file
        self._debug(f"writing '{datasetName}' buffer")
    ...
    def _resizeDataset(self, dataset, dbName, baseSize=0, finished=0):
        ...
        # show the old vs new size of the dataset
        dataset.resize(tuple(shape))
        self._debug(f"old size of '{dbName}': {origSize:,}; new size: {newSize:,}.")
