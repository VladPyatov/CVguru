...
class SpatialVerifier:
    def __init__(self, featuresDBPath, idf, vocab, ratio=0.8, minMatches=10, reprojThresh=4.0):
        # store the inverted document frequency array, visual vocabulary, ratio for feature matching,
        # minimum number of matches required to perform spatial verification, and the projection threshold
        # to for computing inlier points
        self.idf = idf
        self.vocab = vocab
        self.ratio = ratio
    ...
    def rerank(self, queryKps, queryDescs, searchResult, numResults=10):
        ...
        resultsIdxs = np.array([r[-1] for r in searchResult.results])
        resultsIdxs.sort()

        # loop over the starting and ending indexes into the features dataset for each image
        for i, (start, end) in zip(resultsIdxs, self.featuresDB["index"][resultsIdxs, ...]):
            # grab the rows in from the features dataset and break the rows into keypoints and feature vectors
            rows = self.featuresDB["features"][start:end]
            (kps, descs) = (rows[:, :2], rows[:, 2:])

            # determine matched inlier keypoints and grab the indexes of the matched keypoints
            # into the bag-of-visual-words
            bovwIdxs = self.match(queryKps, queryDescs.astype("float32"), kps, descs.astype("float32"))

            # provided that at least some keypoints were matched, the final score for the spatial
            # verification is the sum of the idf values for the inlier words
        ...
        for (score, imageID, imageIdx) in searchResult.results:
            # only add the initial result to the list of results if
            # the image has NOT been spatially verified
    ...
    def match(self, kpsA, featuresA, kpsB, featuresB):
        ...
        if len(matches) >= self.minMatches:
            ...
            # compute the homography between the two sets of points and
            # compute the ratio of of matched points
            (_, status) = cv2.findHomography(ptsA, ptsB, cv2.RANSAC, self.reprojThresh)

            # grab the inlier points (i.e., where the status > 0), then
            # quantize the features associated with these points
