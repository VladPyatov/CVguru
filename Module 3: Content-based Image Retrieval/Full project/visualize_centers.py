...
# initialize the visualization dictionary and initialize the progress bar
...
for (i, imageID) in enumerate(featuresDB["image_ids"]):
    ...
    for kp, features in zip(kps, descs):
        ...
        for j in np.arange(0, len(vocab)):
            ...
            # the distance, keypoint and imageID
            topResults = vis[j]
            ...
            topResults = sorted(topResults, key=lambda x: x[0])[:16]
            vis[j] = topResults
