...
# change the directory to where Hadoop lives
cd $HADOOP_HOME

# (potentially optional): turn off the mode
bin/hdfs dfsadmin -safemode leave

# remove the previous output directory
bin/hdfs dfs -rm -r /user/guru/faces/output

# define the set of local files that need to be present
# to run the Hadoop -- comma separate each file path
FILES="${BASE}/face_detector_mapper.py,\
${BASE}/deploy/project.zip,\
