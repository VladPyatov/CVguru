...
class BOVWIndexer(BaseIndexer):
    def __init__(self, fvectorSize, dbPath, estNumImages=500, maxBufferSize=500, dbResizeFactor=2, verbose=True):
        ...
        # open HDF5 database fro writing, initialize the datasets within the group,
    ...
    def _createDatasets(self):
        # grab the feature vector and create the dataset
        self._debug("creating datasets...")
        self.bovwDB = self.db.create_dataset("bovw", (self.estNumImages, self.fvectorSize),
    ...
    def df(self, method=None):
        ...
        # otherwise, a valid method was supplied, so return the raw document frequency count
        return self._df
