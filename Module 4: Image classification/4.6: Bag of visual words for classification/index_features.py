...
from project.indexer import FeatureIndexer
...
for i, imagePath in enumerate(imagePaths):
    ...
    if i > 0 and i % 10 == 0:
        fi._debug(f"processed {i} images", msgType="[PROGRESS]")

    # extract the image filename and class from the image path and use it to construct the unique image ID
    p = imagePath.split("/")
    imageID = f"{p[-2]}:{p[-1]}"

    # load the image and prepare it to description
